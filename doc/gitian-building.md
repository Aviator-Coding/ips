...
*Setup instructions for a gitian build of IPS using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IPS
...
```bash
apt-get install make curl git ruby sudo apt-cacher-ng qemu-utils debootstrap lxc python-cheetah parted kpartx bridge-utils
adduser debian sudo
```
...
Clone the git repositories for ips and gitian and then checkout the ips version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ips-crypto/ips.git
cd ips
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ips repository
...
To build IPS (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ips repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ips/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ips-crypto/ips
...
./bin/gbuild --commit ips=${COMMIT} --url ips=${URL} ../ips/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ips-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ips-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ips-build.assert
```
...
[ips/gitian.sigs](https://github.com/ips-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
